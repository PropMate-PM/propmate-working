...
const templates = {
  ...
  noreply: {
    welcome: ({ to, name }) => {
      const content = `
        ...
        <p>Thanks for joining ${APP_NAME}. Start saving on prop firm challenges and earn cashback.</p>
        <p><a class="btn" href="https://${DOMAIN}">Go to Dashboard</a></p>
      `
      const html = baseTemplate({ title: 'Welcome', content, unsubscribeUrl: unsubscribeUrlFor(to) })
      return { subject: `Welcome to ${APP_NAME}!`, html, text: textify(html) }
    },
  ...
  payments: {
    ...
    payoutProcessing: ({ to, amount, method }) => {
      const content = `
        <h2>Payout Processing</h2>
        <p>Your payout of <strong>$${Number(amount).toFixed(2)}</strong> is now processing via <strong>${method}</strong>.</p>
      `
      const html = baseTemplate({ title: 'Payout Processing', content, unsubscribeUrl: unsubscribeUrlFor(to) })
      return { subject: `Payout processing: $${Number(amount).toFixed(2)}`, html, text: textify(html) }
    },
